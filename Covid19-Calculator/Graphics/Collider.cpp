...
DEBUG_CODE(extern size_t CollisionTestCounter;)
...
SphereCollider::SphereCollider(Sprite *_sprite)
	...
	(
		NULL,
		_sprite->Position,
		(float)std::min((int)((float)_sprite->Size.x * 0.35f), // What... The fuck did I do here and why??? Oh Size was iVec2
...
SphereCollider::SphereCollider(GameObject *_parent)
	...
	(
		...
		(float)std::min((int)((float)_parent->g_Sprite().Size.x * 0.35f), // What... The fuck did I do here and why??? Oh Size was iVec2
...
{/// We should return the Collision Pair in the future but this is good enough for now
	...
	{// Test the Collider against all other Colliders in _test
		if (Other->hasCollider() == false)
		{
			continue;
		}


		if( (g_Group() != Other->g_Collider().g_Group()) && (Other->Alive() == true) )
...
{
	return { std::min(_first.x,_second.x), std::min(_first.y, _second.y) };
}
Vec2 Collider::maxVec2(Vec2 _first, Vec2 _second)
{
	return { std::max(_first.x,_second.x), std::max(_first.y, _second.y) };
}
